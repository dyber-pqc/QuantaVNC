...
static const char *about_text()
{
  ...
  snprintf(buffer, sizeof(buffer),
           _("QuantaVNC v%s (Based on TigerVNC)\n"
             "Built on: %s\n"
             "Copyright (C) 2026 Dyber, Inc.\n"
             "Copyright (C) 1999-%d TigerVNC team and many others (see README.rst)\n"
             "See https://github.com/dyber-pqc/QuantaVNC for information on QuantaVNC."),
...
{
  fl_message_title(_("About QuantaVNC"));
...
static void CleanupSignalHandler(int sig)
{
  ...
  vlog.info(_("Termination signal %d has been received. QuantaVNC will now exit."), sig);
  exit(1);
}
...
static void init_fltk()
{
  ...
  for (int icon_size : icon_sizes) {
      ...
      sprintf(icon_path, "%s/icons/hicolor/%dx%d/apps/quantavnc.png",
  ...
  fl_message_title_default("QuantaVNC");
...
static void usage(const char *programName)
{
  ...
  fprintf(stderr, _("\n"
          ...
          "  -geometry geometry - Initial position of the main QuantaVNC window. See the\n"
...
create_base_dirs()
{
  ...
  if (vncdir != nullptr && strcmp(vncdir, "vnc") == 0)
    vlog.info(_("%%APPDATA%%\\vnc is deprecated, please switch to the %%APPDATA%%\\QuantaVNC location."));
#endif
