...
OptionsDialog::OptionsDialog()
  : Fl_Window(580, 420, _("QuantaVNC options"))
{
  int x, y;
...
void OptionsDialog::createSecurityPage(int tx, int ty, int tw, int th)
{
  ...
  ty += GROUP_LABEL_OFFSET;
  Fl_Group *pqcGroup = new Fl_Group(tx, ty, width, 0,
                                     _("Post-Quantum Cryptography"));
  pqcGroup->labelfont(FL_BOLD);
  pqcGroup->box(FL_FLAT_BOX);
  pqcGroup->align(FL_ALIGN_LEFT | FL_ALIGN_TOP);
  ...
  pqcGroup->end();
  pqcGroup->resizable(nullptr);
  pqcGroup->size(pqcGroup->w(), ty - pqcGroup->y());
