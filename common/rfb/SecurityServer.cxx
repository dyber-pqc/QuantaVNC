...
#endif
#ifdef HAVE_LIBOQS
 ", PQKEMNone, PQKEMVnc, PQKEMPlain"
#endif
#if defined(HAVE_LIBOQS) && defined(HAVE_GNUTLS)
...
#endif
#ifdef HAVE_LIBOQS
  case secTypePQKEMNone:
    return new SSecurityPQKEM(sc, secTypePQKEMNone, true);
  case secTypePQKEMVnc:
    return new SSecurityPQKEM(sc, secTypePQKEMVnc, true);
  case secTypePQKEMPlain:
    return new SSecurityPQKEM(sc, secTypePQKEMPlain, true);
#endif
#if defined(HAVE_LIBOQS) && defined(HAVE_GNUTLS)
  case secTypePQTLSNone:
    return new SSecurityStack(sc, secTypePQTLSNone,
                              new SSecurityTLS(sc, true, true));
  case secTypePQTLSVnc:
    return new SSecurityStack(sc, secTypePQTLSVnc,
                              new SSecurityTLS(sc, true, true),
                              new SSecurityVncAuth(sc));
  case secTypePQTLSPlain:
    return new SSecurityStack(sc, secTypePQTLSPlain,
                              new SSecurityTLS(sc, true, true),
                              new SSecurityPlain(sc));
  case secTypePQX509None:
    return new SSecurityStack(sc, secTypePQX509None,
                              new SSecurityTLS(sc, false, true));
  case secTypePQX509Vnc:
    return new SSecurityStack(sc, secTypePQX509Vnc,
                              new SSecurityTLS(sc, false, true),
                              new SSecurityVncAuth(sc));
  case secTypePQX509Plain:
    return new SSecurityStack(sc, secTypePQX509Plain,
                              new SSecurityTLS(sc, false, true),
                              new SSecurityPlain(sc));
#endif
  }
