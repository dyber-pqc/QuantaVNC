...
SSecurityTLS::SSecurityTLS(SConnection* sc_, bool _anon, bool requirePQC_)
  : SSecurity(sc_), session(nullptr), anon_cred(nullptr),
    cert_cred(nullptr), anon(_anon), requirePQC(requirePQC_), tlssock(nullptr),
...
void SSecurityTLS::setParams()
{
  ...
  // PQC hybrid key exchange group support
  if (requirePQC) {
    // Mandatory PQC: prepend ML-KEM hybrid group to priority
    const char *err;
    std::string pqcPrio = "NORMAL:+GROUP-X25519-MLKEM768";
    if (anon) {
      pqcPrio += ":";
      pqcPrio += kx_anon_priority;
    }
    ret = gnutls_priority_set_direct(session, pqcPrio.c_str(), &err);
    if (ret != GNUTLS_E_SUCCESS) {
      if (ret == GNUTLS_E_INVALID_REQUEST)
        vlog.error("PQC GnuTLS priority syntax error at: %s", err);
      throw rdr::tls_error("gnutls_priority_set_direct() [PQC required]", ret);
    }
    vlog.debug("PQC hybrid key exchange required (X25519-MLKEM768)");
  }
#ifdef HAVE_LIBOQS
  else {
    // Opportunistic PQC: try PQC groups, fall back to standard if unsupported
    const char *err;
    std::string pqcPrio = "NORMAL:+GROUP-X25519-MLKEM768";
    if (anon) {
      pqcPrio += ":";
    ...
    ret = gnutls_priority_set_direct(session, pqcPrio.c_str(), &err);
    if (ret != GNUTLS_E_SUCCESS) {
      vlog.info("PQC groups not supported by GnuTLS, falling back to standard");
      // Fall back: re-apply standard priority (already set above)
      std::string stdPrio = "NORMAL";
      if (anon) {
        stdPrio += ":";
        stdPrio += kx_anon_priority;
      }
      ret = gnutls_priority_set_direct(session, stdPrio.c_str(), &err);
      if (ret != GNUTLS_E_SUCCESS) {
        if (ret == GNUTLS_E_INVALID_REQUEST)
          vlog.error("GnuTLS priority syntax error at: %s", err);
        throw rdr::tls_error("gnutls_priority_set_direct() [fallback]", ret);
      }
    } else {
      vlog.debug("PQC hybrid key exchange enabled opportunistically (X25519-MLKEM768)");
    }
  }
#endif
