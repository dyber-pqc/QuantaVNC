...
uint32_t rfb::secTypeNum(const char* name)
{
  ...
  /* QuantaVNC PQC subtypes */
  if (strcasecmp(name, "PQKEMNone") == 0)    return secTypePQKEMNone;
  if (strcasecmp(name, "PQKEMVnc") == 0)     return secTypePQKEMVnc;
  if (strcasecmp(name, "PQKEMPlain") == 0)    return secTypePQKEMPlain;
  if (strcasecmp(name, "PQTLSNone") == 0)     return secTypePQTLSNone;
  if (strcasecmp(name, "PQTLSVnc") == 0)      return secTypePQTLSVnc;
  if (strcasecmp(name, "PQTLSPlain") == 0)     return secTypePQTLSPlain;
  if (strcasecmp(name, "PQX509None") == 0)     return secTypePQX509None;
  if (strcasecmp(name, "PQX509Vnc") == 0)      return secTypePQX509Vnc;
  if (strcasecmp(name, "PQX509Plain") == 0)     return secTypePQX509Plain;
